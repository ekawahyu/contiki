...
ota_restore_map()
{
  uint32_t data;
  uint32_t seg;
  for(seg = OTA_FLASH_START; seg < OTA_FLASH_END; seg += OTA_SEGMENT_MAX) {
 ...
 */
uint8_t
ota_next_segment(uint16_t img_size, uint16_t *next_addr)
{
...
{
  uint32_t pg;
  for(pg = OTA_FLASH_START; pg < OTA_FLASH_END; pg += FLASH_PAGE_SIZE)
    flash_page_erase(FLASH_PAGE(pg));
  ota_init();
}
/*---------------------------------------------------------------------------*/
void
ota_flashwrite(uint16_t addr, uint8_t size, uint8_t *data)
{
    flash_write_DMA(data, size, FLASH_WORD_ADDR((uint32_t)(OTA_FLASH_START + addr)));
}
/*---------------------------------------------------------------------------*/
void
ota_copyandreset(void * arg)
{
  // erase existing "code"
 ...
 */
uint8_t
ota_verify_crc(uint16_t size, uint16_t crc)
{
  return 1;
}
